...
echo Generating post-market carousel + copy kit...
echo.
.\.venv\Scripts\python.exe "Content Creation\main.py" --now post --dry-run
echo.

if exist "%OUTDIR%\copy_post_market.txt" (
    echo ============================================
    echo   Copy kit generated:
    echo   %OUTDIR%\copy_post_market.txt
    echo ============================================
    echo.
    type "%OUTDIR%\copy_post_market.txt"
    echo.
) else (
